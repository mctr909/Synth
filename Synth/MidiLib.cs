...
public abstract class MidiIn : MidiLib {
	...
	delegate void DCallback(IntPtr hMidiIn, MidiInMessage uMsg, IntPtr dwInstance, uint dwParam1, uint dwParam2);
	...
	[DllImport("winmm.dll", SetLastError = true, CharSet = CharSet.Auto)]
	static extern MMRESULT midiInPrepareHeader(IntPtr hmi, IntPtr lpMidiInHdr, int size);
	[DllImport("winmm.dll")]
	static extern MMRESULT midiInUnprepareHeader(IntPtr hmi, IntPtr lpMidiInHdr, int size);
	[DllImport("winmm.dll", SetLastError = true, CharSet = CharSet.Auto)]
	static extern MMRESULT midiInReset(IntPtr hmi);
	[DllImport("winmm.dll", SetLastError = true, CharSet = CharSet.Auto)]
	static extern MMRESULT midiInAddBuffer(IntPtr hmi, IntPtr lpMidiInHdr, int size);
	[DllImport("winmm.dll", SetLastError = true, CharSet = CharSet.Auto)]
	...
	public MidiIn(int bufferSize = 1024, int bufferCount = 4) : base(bufferSize, bufferCount) { }
	...
	void Callback(IntPtr hMidiIn, MidiInMessage uMsg, IntPtr dwInstance, uint dwParam1, uint dwParam2) {
		switch (uMsg) {
